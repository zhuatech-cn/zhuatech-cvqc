 ...
 */
@SpringBootTest @AutoConfigureMockMvc class CvqcApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("计划检测数量"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanSubmitProductionReport()throws Exception{mvc.perform(post("/api/shopfloor/work-orders/1/reports").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"operationName\":\"缺陷复核\",\"goodQty\":2,\"defectQty\":1,\"remark\":\"图像清晰\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.message").value("反馈提交成功")).andExpect(jsonPath("$.data.completedQty").value(4218));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanDecideVisualDefectDisposition()throws Exception{mvc.perform(post("/api/shopfloor/visual-disposition").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"defectType\":\"制动部件裂纹\",\"confidence\":0.93,\"defectAreaRatio\":0.18,\"safetyCritical\":true,\"repeatCount\":2}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.disposition").value("STOP_LINE")).andExpect(jsonPath("$.data.severity").value("CRITICAL")).andExpect(jsonPath("$.data.riskScore").value(88)).andExpect(jsonPath("$.data.sampleExpansion").value(15));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
